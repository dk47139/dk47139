...
if (php_sapi_name() === 'cli' || $cli = apiRequest()) {
  ...
  $data['uuid'] = (!empty($data['uuid']) && preg_match('~^([a-z0-9]{4}[-]?){3}[a-z0-9]{4}$~i', $data['uuid'])) ? strtoupper(preg_replace('~[^a-z0-9]~i', '', $data['uuid'])) : '';
  ...
  switch ($cli['action']) {
    case 'help':
      showWarningJson([
        ...
        'actions' => [
          ...
          'perform.import'             => 'data: uuid, hostnames, overwrite<all|none|newer>, submerge<0|1>, subdomain, acms_settings<0|1>, loader_settings<0|1>, custom_includes<0|1>, templates<0|1>',
    ...
    case 'plugin.deactivate' :
      ...
      break;
    case 'change.uuid' :
      if (!empty($data['uuid']) && preg_match('~([0-9A-X]{4}-?){4}~i', $data['uuid'])) {
...
function importFlatFile($params = [])
{
  ...
  $include = [];
  if (!empty($params['include'])) $include = array_merge($include, array_filter(explode(',', $params['include'])));
  if (!empty($params['exclude'])) $exclude = array_merge($exclude, array_filter(explode(',', $params['exclude'])));
  //print_r( $include );
  //print_r( $exclude );

  $filterInclude = function ($file, $key, $iterator) use ($include) {
    if (!count($include)) return true;
  ...
  $filterExclude = function ($file, $key, $iterator) use ($exclude) {
  ...
  foreach ($iterator as $file) {
    ...
    $path = preg_replace('~^' . preg_quote($source) . '~', '', $file->getPathname());
    if (pathExists($uuidSettings['domain'], $path)) {
      $stats['skipped']++;
      continue;
    }
    if ($charset = mb_detect_encoding(file_get_contents($file->getPathname()))) $charset = strtolower($charset);
    $rowid = createUrl([
      'hostname' => $uuidSettings['domain'],
      'path'     => $path,
      'mime'     => $mime['mimetype'],
      'charset'  => $charset,
    ]);
            ...
            if (inExpertMode() && $section == 'import.urls') { ?>
              <form action="" method="post" class="import-external-urls-wrapper">
                ...
                <div class="form-group">
                  ...
                  <textarea class="form-control codemirror-editor" data-mime="text/plain"
                            data-readonly="false"
                            name="externalUrlsList"><?=(!empty($_POST['externalUrlsList']) ? htmlspecialchars($_POST['externalUrlsList']) : '')?></textarea>
                </div>
              </form>
            <?php }
                  ...
                  <div class="row">
                    <div class="col-12 col-md-3 mb-3">
                      <div class="border border-danger alert-danger rounded w-100 text-center p-3 embed-responsive embed-responsive-16by9 d-flex">
                                                <span class="h5 text-monospace align-self-center w-100 text-danger"><i
    ...
    <?php if ($section == 'search') { ?>
      ...
      <div class="modal fade" id="reservedVarsModal" tabindex="-1" role="dialog" aria-labelledby="templateLabel"
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
          <div class="modal-content">
            <div class="modal-header bg-primary text-light">
              <h5 class="modal-title"><?=L('Reserved Vars Cheat Sheet')?></h5>
              <button type="button" class="close text-light" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              ...
              <div class="row">
                <div class="col-12 mb-3">
                  ...
                  <kbd>%%filesize%</kbd> <?=L('int file size in bytes')?><br>
                  <kbd>%%filetime%</kbd> <?=L('used snapshop timestamp in YYYYMMDDHHMMSS format')?>
                  <br>
                  ...
                  <kbd>%%depth%</kbd> <?=L('assigned depth int value by the paged depth tool')?><br>
                </div>
                <div class="col-12 mb-3">
